...
protected:
	...
	inline FDelegateHandle AddInternal( IDelegateInstance* DelegateInstance )
	{
		...
		InvocationList.push_back(DelegateInstance);
	...
	{
		if ((InvocationList.size() < CompactionThreshold) || (InvocationListLockCount != 0))
		{
			return;
		}

		for (int32_t InvocationListIndex = InvocationList.size() - 1; InvocationListIndex >= 0; --InvocationListIndex)
		{
			...
			if ((DelegateInstance == nullptr) || DelegateInstance->IsCompactable())
			{
				...
				InvocationList.erase(InvocationList.begin() + InvocationListIndex);
		...
		CompactionThreshold = (std::max)(2, 2 * InvocationList.size());
	}
	 ...
	 */
	inline const std::vector<IDelegateInstance*>& GetInvocationList( ) const
	{
...
private:
	...
	std::vector<IDelegateInstance*> InvocationList;

	/** Holds a lock counter for the invocation list. */
	mutable int32_t InvocationListLockCount;
};
